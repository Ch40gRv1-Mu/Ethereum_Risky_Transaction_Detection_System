...
library ECC {
    ...
    function verifyECRecover(bytes32 messageHash, bytes  memory publicKey, bytes memory signature) public pure returns (bool correctlySigned) {
        ...
        address suppliedAddress = address(bytes20(keccak256(abi.encodePacked(p1, p2))));
    ...
    // Signature is the signature that was created over the hash of message
    // PublicKey is the uncompressed x,y representation of the key which should be used for verification
    function verifySECP256R1(bytes32 messageHash, bytes memory publicKey, bytes memory signature) public pure returns (bool correctlySigned) {
